...
{
    $instructor = Instructor::findOrFail($d);
    $role = Role::select('id','name')->distinct()->get();

    // casts в модели уже вернут массивы, json_decode не нужен
    return view('backend.instructor.edit', compact('instructor','role'));
}

public function update(UpdateRequest $request, $d)
{
    DB::beginTransaction();
    try {
        $instructor = Instructor::findOrFail($d);
        ...
        // Сохраняем переводы так же, как в store
        $instructor->name = json_encode([
            'en' => $request->name['en'] ?? null,
            'ru' => $request->name['ru'] ?? null,
            'ka' => $request->name['ka'] ?? null,
