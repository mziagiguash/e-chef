            ...
            <div class="col-lg-12">
                <div class="card">
                    <div class="card-header">
                        <h4 class="card-title">Edit Quiz Information</h4>
                    </div>
                    <div class="card-body">
                        <form action="{{ localeRoute('quiz.update', ['quiz' => encryptor('encrypt', $quiz->id)]) }}" method="POST">
                            @csrf
                            @method('PUT')
                                    ...
                                    <div class="tab-pane fade {{ $localeCode === $currentLocale ? 'show active' : '' }}"
                                        ...
                                        <div class="form-group mt-3">
                                            ...
                                            <input type="text" class="form-control" id="title_{{ $localeCode }}"
                                                   ...
                                                   value="{{ old("title_$localeCode", $translation->title ?? '') }}"
                                                   required>
                                        </div>
                                        <div class="form-group">
                                            <label for="description_{{ $localeCode }}">Description ({{ $localeName }})</label>
                                            <textarea class="form-control" id="description_{{ $localeCode }}"
                                                      name="description_{{ $localeCode }}" rows="4">{{ old("description_$localeCode", $translation->description ?? '') }}</textarea>
                                        </div>
                                    </div>
                                @endforeach
                            </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        $('form').on('submit', function() {
            ...
            // Проверяем обязательные поля для всех языков
            @foreach($locales as $localeCode => $localeName)
