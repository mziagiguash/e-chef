...
$coursePrerequisites = $course->prerequisites ?? 'No prerequisites specified';
$lessonsCount = isset($course->lessons) ? $course->lessons->count() : 0;
$hasInstructor = isset($course->instructor) && $course->instructor;
?>
...
<div class="tab-pane fade" id="nav-linstruc" role="tabpanel" aria-labelledby="nav-linstruc-tab">
            ...
            <div class="course-instructor mw-100">
                <div class="course-instructor-info">
                    <div class="instructor-image">
                        <img src="{{ isset($course->instructor->image) ? asset('uploads/users/'.$course->instructor->image) : asset('frontend/dist/images/ellipse/user.jpg') }}"
                             alt="Instructor" />
                    </div>
                    <div class="instructor-text">
                        ...
                        <p>{{ $course->instructor->designation ?? 'Instructor' }}</p>
                        <!-- остальной код интсруктора -->
                    </div>
                </div>
                <p class="lead-p">{{ $course->instructor->title ?? 'Instructor title not available' }}</p>
                <p class="course-instructor-description">
                    {{ $course->instructor->bio ?? 'Instructor bio not available' }}
                </p>
            </div>
        </div>
    </div>
    ...
    <h3 class="font-title--sm">
        {{ $course->price ? $currencySymbol . number_format($course->price * $currencyRate, 2) : 'Free' }}
    </h3>
    <p>
        <del>
            {{ $course->old_price ? $currencySymbol . number_format($course->old_price * $currencyRate, 2) : '' }}
        </del>
    </p>
</div>
