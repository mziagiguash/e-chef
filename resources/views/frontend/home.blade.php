...
<a href="{{ localeRoute('courseDetails', ['id' => encryptor('encrypt', $pc->id)]) }}"
   ...
   {{ $pc->translated_title }}
        ...
        {{ $pc->instructor?->translated_name ?? 'No Instructor' }}
    </p>
</a>
                                        <div class="price">
                                             <del>$95</del>
...
<section class="section best-instructor-featured overflow-hidden main-instructor-featured">
    ...
    @php
        $imagePath = 'uploads/users/instructor_' . $i->id . '.*';

    @endphp

    @if(file_exists(public_path($imagePath)))
    <img src="{{ asset($imagePath) }}" alt="{{ $i->translated_name ?? 'Instructor' }}" />
    @else
    ...
    <a href="{{ localeRoute('instructorProfile', encryptor('encrypt', $i->id)) }}" tabindex="0">
        {{ $i->translated_designation }}
    </a>
    <p>{{ $i->translated_name }}</p>
